...
const tableEditOperation = {
    openEditRowModal: function (table, editOperation) {
        let thisHelper = this;
        //Set editOpData
        let editOpData = JSON.parse(JSON.stringify(editOperation.data));
        editOpData =  tableResolveDeep(editOpData, table, `selectedRow`);
        //Open Modal
        commonFunctions.openModal(500, 640, thisHelper.fillEditRowModal(editOpData, editOperation));
        //Input controls
        thisHelper.inputControls(editOpData);
        //Edit Row Button
        thisHelper.editRowButtonClick(table, editOperation, editOpData);
    },

    fillEditRowModal: function(editOpData, editOperation) {
        let modalHtml = ``;
        let formHtml = '';
        ...
        modalHtml = `<form class="w-full h-full flex flex-col justify-between items-center gap-4">
                            <h1 class="w-full text-2xl font-bold">${t(editOperation.title)}</h1>
                            <div class="w-full h-full overflow-y-auto flex flex-col gap-4">${formHtml}</div>
                            <button type="button" id="addRowButton" class="w-full text-xl font-bold py-2 rounded-md bg-main text-white tracking-widest">${t('defaults.save')}</button>
                        </form>`;
        return modalHtml;
    },

    inputControls: function(editOpData) {
        editOpData.forEach(item => {
            if (item.visible) {
    ...
    editRowButtonClick: function(table, editOperation, editOpData) {
...
export default tableEditOperation;